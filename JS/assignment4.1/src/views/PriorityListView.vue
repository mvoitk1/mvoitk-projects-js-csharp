...
async function create() {
  error.value = null
  try {
    await store.create({ priorityName: newName.value, prioritySort: newSort.value })
    newName.value = ''
    newSort.value = 0
  } catch {
...
<template>
  <div class="page">
    ...
    <ul class="list">
      <li v-for="p in store.items" :key="p.id" class="item">
        <span>{{ p.priorityName }}</span>
        <span class="meta">sort: {{ p.prioritySort }}</span>
        <button class="btn-del" @click="remove(p.id)">Delete</button>
      </li>
      <li v-if="store.items.length === 0 && !store.loading">No priorities yet.</li>
    </ul>
