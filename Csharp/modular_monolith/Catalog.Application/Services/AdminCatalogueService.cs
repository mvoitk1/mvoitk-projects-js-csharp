...
public class AdminCatalogueService(ICatalogUnitOfWork uow, IMediator mediator) : IAdminCatalogueService
{
    // ─── Categories ──────────────────────────────────────────────────────────
    ...
    public async Task<DashboardStatsDto> GetDashboardStatsAsync()
    {
        ...
        var orderStats = await mediator.Send(new GetOrderStatsQuery(cutoff));

        return new DashboardStatsDto(
            products.Count(),
            orderStats.TotalCount,
            variants.Count(v => v.StockQty < 5 && v.IsActive),
            orderStats.RecentCount);
    }
